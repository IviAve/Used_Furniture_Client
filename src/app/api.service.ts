...
export class ApiService {
  ...
  getLastThreeFurnitures(limit?: number): Observable<Furniture[]> {
    ...
    return this.http.get<Furniture[]>(url, { withCredentials: true }).pipe(
      catchError((error) => {
        console.error('Error fetching last three furnitures', error);
        throw error;
      })
  ...
  getFurnitures() {
    return this.http.get<Furniture[]>(`/api/furnitures`, { withCredentials: true });
  }

  getSingleFurniture(furnitureId: string) {
    return this.http.get<Furniture>(`/api/furnitures/${furnitureId}`, { withCredentials: true });
  }

  createFurniture(category: string, condition: string, delivery: string, location: string, phone: string, imageUrl: string, summary: string) {
    const payload = { category, condition, delivery, location, phone, imageUrl, summary };
    return this.http.post<Furniture>(`/api/furnitures`, payload, { withCredentials: true });
  }
  ...
  deleteFurniture(furnitureId: string) {
    return this.http.delete(`/api/furnitures/${furnitureId}`, { withCredentials: true });
  }
  ...
  getTransports() {
    return this.http.get<Transport[]>(`/api/transports`, { withCredentials: true });
  }

  getSingleTransport(transportId: string) {
    return this.http.get<Transport>(`/api/transports/${transportId}`, { withCredentials: true });
  }
  ...
  createTransport(
    ...
    const payload = { name, type, capacity, price, location, phone, imageUrl, summary };
    return this.http.post<Transport>(`/api/transports`, payload, { withCredentials: true });
  }
  ...
  deleteTransport(transportId: string) {
    return this.http.delete(`/api/transports/${transportId}`, { withCredentials: true });
  }

  likeFurniture(furnitureId: string) {
    return this.http.put(`/api/furnitures/${furnitureId}/subscribe`, {}, { withCredentials: true });
  }

  likeTransport(transportId: string) {
    return this.http.put(`/api/transports/${transportId}/subscribe`, {}, { withCredentials: true });
  }
}
